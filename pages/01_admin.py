...
def log_action(conn, action, details):
    ...
    with conn.session as session:
        session.execute(
            text("INSERT INTO logs (date, action, details) VALUES (:date, :action, :details)"),
    ...
    try:
        ...
        with conn.session as session:
            for ing in data:
                ...
                session.execute(
                    text("INSERT INTO ingredients (name, unit_ok) VALUES (:name, :unit_ok)"),
                ...
                with conn.session as session:
                    session.execute(
                        text("""
                            ...
                            WHERE name = :old_name
                        """),
                        {"name": new_name, "unit_ok": int(unit_ok), "old_name": selected}
                    )
                    session.commit()
                    log_action(conn, "update", f"Updated {selected} to {new_name} (unit_ok: {unit_ok})")
                    # st.rerun()
